...
__weak void SX1278_hw_init(SX1278_hw_t * hw) {
    ...
    HAL_GPIO_WritePin((GPIO_TypeDef *)hw->reset.port, hw->reset.pin, GPIO_PIN_SET);
}

__weak void SX1278_hw_SetNSS(SX1278_hw_t * hw, int value) {
    HAL_GPIO_WritePin((GPIO_TypeDef *)hw->nss.port, hw->nss.pin,
                      (value == 1) ? GPIO_PIN_SET : GPIO_PIN_RESET);
}

__weak void SX1278_hw_Reset(SX1278_hw_t * hw) {
    SX1278_hw_SetNSS(hw, 1);
    HAL_GPIO_WritePin((GPIO_TypeDef *)hw->reset.port, hw->reset.pin, GPIO_PIN_RESET);
...
__weak void SX1278_hw_SPICommand(SX1278_hw_t * hw, uint8_t cmd) {
    ...
    HAL_SPI_Transmit((SPI_HandleTypeDef *)hw->spi, &cmd, 1, 1000);
    while (HAL_SPI_GetState((SPI_HandleTypeDef *)hw->spi) != HAL_SPI_STATE_READY)
        ;
}
...
SX1278_hw_SetNSS(hw, 0);
HAL_SPI_TransmitReceive((SPI_HandleTypeDef *)hw->spi, &txByte, &rxByte, 1, 1000);
while (HAL_SPI_GetState((SPI_HandleTypeDef *)hw->spi) != HAL_SPI_STATE_READY)
;
return rxByte;
}
...
__weak int SX1278_hw_GetDIO0(SX1278_hw_t * hw) {
    return (HAL_GPIO_ReadPin((GPIO_TypeDef *)hw->dio0.port, hw->dio0.pin) == GPIO_PIN_SET);
}
